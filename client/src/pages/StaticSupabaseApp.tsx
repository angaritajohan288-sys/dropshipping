...
export default function StaticSupabaseApp() {
  ...
  useEffect(() => {
    const phaseHeader = document.querySelector("#static-checklist > article > div:first-child");
    if (!phaseHeader) return;
    phaseHeader.querySelector("#phase-summary-cards")?.remove();
    const summary = document.createElement("div");
    summary.id = "phase-summary-cards";
    summary.innerHTML = `<section class="phase-summary-card phase-deliverable"><span>Entregable</span><p>${phase.deliverable}</p></section><section class="phase-summary-card phase-exit"><span>Puerta de salida</span><p>${phase.exitGate}</p></section>`;
    phaseHeader.appendChild(summary);
    return () => summary.remove();
  }, [phase.id, phase.deliverable, phase.exitGate]);
